...
In the following, an analysis strategy close to the one described in Chapter~\ref{chap:RA2} is pursued. Events selected with the baseline selection are further categorized according to their \HT and \met values in exclusive search regions shown in Tab.~\ref{tab:stop_excl_search_bins}. 
\begin{table}[!t]
...
\section[Performance Comparison to Selection Based on Published Stop Analysis]{Performance Comparison to Selection Based on Published Stop Analysis at $\sqrt{s} = 8$\tev}
\label{sec:stop_pub}
...
\label{sec:stop_syst}
In order to study the stability of the sensitivity of the identified selection towards specific choices and assumptions, the expected limit for the selection using the baseline requirements, at least one CMS Top Tag and $m_\mathrm{T} > 180$\gev is evaluated when varying the binning of the search regions and the assumed \ttbar uncertainty. \\
The behaviour concerning the binning of the search regions is studied by dividing the region $ 200 < \met < 400$\gev into two: $ 200 < \met < 300$\gev and $ 300 < \met < 400$\gev. Here, the systematic uncertainties are considered as before. The dependence on the assumed uncertainties is studied by increasing the uncertainties considered for \ttbar from 20\% to 50\% while the other stay as before. Since \ttbar is for this selection the main background, the variation of the \ttbar uncertainty is supposed to show the main effect. \\
The expected limits for the shifts described above are illustrated together with the nominal result in Fig.~\ref{fig:stop_varied_limit}. It turns out that the resulting changes for the sensitivity caused by the systematic shifts is small and only impacts small stop quark masses.  
\begin{figure}[!t]
  \centering
  \begin{tabular}{c}
...
\end{figure}
\\
Consequently, the derived selection is able to probe the top squark mass region up to 1\tev even for a much higher uncertainty on the \ttbar background. Accordingly, if the \ttbar background can not be determined in data to a precision of 20\%, the sensitivity of the analysis is supposed to be not much degraded for high stop quark masses. Furthermore, the variation of the search region binning has shown that it is not expected that much more sensitivity can be gained by increasing the number of exclusive search regions. Thus, another improvement of the analysis would require to identify further kinematic selections.                   
