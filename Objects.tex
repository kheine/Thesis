...
\begin{figure}[!h]
  ...
  \end{tabular}
  \caption{MC-Truth L2L3 corrections for AK5 PF jets as a function of jet pseudorapidity for three reference transverse momentum values: 30\gev (white hollow circles), 100\gev (red squares) and 300\gev (blue circles)~\cite{CMS-DP-2013-033}.}
  \label{fig:l2l3}
\end{figure}
...
\begin{figure}[!h]
  ...
  \end{tabular}
  \caption{L2 residual corrections, obtained from dijet events, for AK5 PF jets, as a function of jet pseudorapidity shown with JES uncertainty (yellow band) and the statistical uncertainty (blue band)~\cite{CMS-DP-2013-033}.}
  \label{fig:l2res}
\end{figure}
...
\end{figure}
As discussed in Sec.~\ref{subsec:susy_collider}, the decay of the top-quark is experimentally characterized by the decay of the W boson and two thirds of top quark decays result exclusively in hadrons. This decay mode is here referred to as \textit{hadronic top}. If top quarks are produced with $\pt \ll m_t$, the top quark decay products show up as three distinct objects in the detector. In the case of hadronic tops, these are three well separated jets. However, if the top transverse momentum is high, the decay products are \textit{boosted} and thus collimated in the forward direction. Consequently, they might overlap and merge into a single large jet (\textit{fat jet}). The opening angle of the decay products $\Delta R$ is expected to scale as
\begin{equation}
...
\end{description} 
In order to identify decays of boosted hadronic top quarks, as discussed above, several top tagging algorithms (or short \textit{taggers}) are commissioned within the CMS experiment~\cite{CMS:2014fya, CMS-DP-2014-036}. Two of them are described in some detail in the following:
\begin{description}
\begin{figure}[!tp]
  \centering 
  \begin{tabular}{cc}
